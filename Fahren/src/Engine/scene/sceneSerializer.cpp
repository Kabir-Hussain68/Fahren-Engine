...
static std::string rigibBody2DBodyTypeToString(RigidBody2DComponent::BodyType type)
{
    switch (type)
    {
    ...
    FH_CORE_ASSERT(false, "Unkwon RigidBody type");
    return {};
}

static RigidBody2DComponent::BodyType rigibBody2DBodyTypeFromString(const std::string typeString)
{
...
static void serializeEntity(YAML::Emitter& out, Entity entity)
{
	...
	if (entity.hasComponent<CameraComponent>())
	{
		...
		out << YAML::Key << "PerspectiveFOV" << YAML::Value << camera.getPrespectiveVerticalFOV();
		out << YAML::Key << "PerspectiveNear" << YAML::Value << camera.getPrespectiveNearClip();
		out << YAML::Key << "PerspectiveFar" << YAML::Value << camera.getPrespectiveFarClip();
    ...
    if (entity.hasComponent<RigidBody2DComponent>())
	{
		...
		out << YAML::Key << "BodyType" << YAML::Value << rigibBody2DBodyTypeToString(rigidBody2DComponent.type);
...
{
    FH_CORE_ASSERT(false, "Not implemented yet");
}
...
bool SceneSerializer::deserialize(const std::string &filepath)
{
    ...
    FH_CORE_TRACE("Deserializing Scene '{0}'", sceneName);
    ...
    if (entities)
    {
        for (auto entity : entities)
        {
			...
			if (cameraComponent)
			{
				...
				cc.camera.setPrespectiveVerticalFOV(cameraProps["PerspectiveFOV"].as<float>());
				cc.camera.setPrespectiveNearClip(cameraProps["PerspectiveNear"].as<float>());
				cc.camera.setPrespectiveFarClip(cameraProps["PerspectiveFar"].as<float>());
			...
			if (rigidBody2DComponent)
			{
                ...
                rb2d.type = rigibBody2DBodyTypeFromString(rigidBody2DComponent["BodyType"].as<std::string>());
